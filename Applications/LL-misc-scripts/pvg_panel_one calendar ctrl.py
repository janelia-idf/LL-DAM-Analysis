...
class thumbnailPanel(previewPanel):
    ...
    def __init__( self, parent, monitor_number, ThumbnailSize=(320,240) ):           
        ...
        self.mon = int(monitor_number)                   # 0-indexed
...
# %%                                                Show Monitor Numbers            
    def displayNumber(self):
        ...
        text1 = wx.StaticText( self, wx.ID_ANY, '%s' % (self.mon+1), pos)     # mon is 0-indexed
        text1.SetFont(font1)
        if pv.call_tracking: debugprt(self,currentframe(),pgm,'end   ')
...
class panelConfigure(wx.Panel):    
    ...
    def __init__(self, parent):
        ...
        self.MonitorList = ['Monitor %s' % (int(m+1)) for m in range( n_monitors )]     # n_monitors is 1-indexed, m is 0-indexed
...
#        self.start_date = wx.DatePickerCtrl(self, wx.ID_ANY, style = wx.DP_DROPDOWN | wx.DP_SHOWCENTURY)
        self.start_date = cal.CalendarCtrl(self, wx.ID_ANY, wx.DateTime_Now())
        self.Bind(cal.EVT_CALENDAR_SEL_CHANGED, self.onDateTimeChanged, self.start_date)                                                                            # $$$$$$ - set default date to start_datetime

        self.date_time_sizer.Add(self.start_date, wx.EXPAND|wx.ALL, border=5)               # calendar only

        self.date_time_right_sizer = wx.BoxSizer(wx.VERTICAL)                               # date & time display on right
        self.date_label = wx.TextCtrl(self, -1, wx.ALIGN_CENTER_HORIZONTAL, 'click on a day')
        self.date_time_right_sizer.Add(self.date_label, 0, wx.EXPAND|wx.ALL, border=5)  # --- add to datetime row, center panel, lower sizer

        # ---------------------------------------------------------------------  time picker goes below date display
        self.txt_time = wx.StaticText(self, -1, "Time:")
        self.txt_24time = wx.StaticText(self, -1, "24-hr format")
 ...
 #       self.Bind(masked.EVT_TIMEUPDATE, self.onDateTimeChanged, self.start_time)                                                                            # $$$$$$ - set default date to start_datetime
        ...
        self.addWidgets(self.date_time_right_sizer, [self.txt_time, self.start_time, self.spinbtn])
        self.date_time_right_sizer.Add(self.txt_24time, wx.ALIGN_CENTER_HORIZONTAL)

        self.date_time_sizer.Add(self.date_time_right_sizer)
        ...
        self.SetSizerAndFit(lowerSizer)
...
# %%                                                    Apply Button
    ...
    def onDateTimeChanged(self,event):
        ...
        print('$$$$$$ do i really need to print this?')    #        date = self.start_date.GetValue()
        date = self.start_date.GetDate()
#        time = self.start_time.GetValue()
#        time = time.time()
